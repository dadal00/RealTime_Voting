<script>
  ...
  const tracer = opentelemetry.trace.getTracer('counter_frontend', '0.1.0')
...
<main>
  <div
    style="background-color:black; width: 100vw; height: 100vh; position: fixed; top: 0; left: 0; overflow: hidden;"
  >
            ...
            {#each click_animations as animation (animation.id)}
              {#if animation.color === color}
                <span
                  ...
                  on:animationend={() =>
                    (click_animations = click_animations.filter((a) => a.id !== animation.id))}
                >
                  +1
                </span>
              {/if}
